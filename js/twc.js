...
function twc_save_widget_edit()
{
	...
	//getting an array of proper values
...
jQuery(document).ready(function(){
	...
	jQuery('.qtipTwc').qtip({
	     ...
	     hide: { when: { target: jQuery('.widget-liquid-left:first'), event: 'mouseout' } },
	     style: {
	        name: 'dark',
	        padding: '7px 13px',
	        width: {
	...
	jQuery('.add:first').qtip({
	    ...
	    hide: { when: { target: jQuery('#twcAdd'), event: 'mouseout' } },
		position: {
	        corner: {
	           target: 'topMiddle',
	...
	jQuery('.sidebar-name:first h3').qtip({
	    ...
	    hide: { when: { target: jQuery('#twcAvailableWidgets'), event: 'mouseout' } },
		position: {
	        corner: {
	           target: 'topMiddle',
	           tooltip: 'bottomMiddle'
	...
	jQuery('.widget-control-settings:first').qtip({
	    ...
	    hide: { when: { target: jQuery('#twcWidgetControl'), event: 'mouseout' } },
		position: {
	        corner: {
	           target: 'topMiddle',
	           tooltip: 'bottomMiddle'
	...
	jQuery('input[name=default_sidebar]:first').qtip({
	    ...
	    hide: { when: { target: jQuery('#qtipDefaultWidget'), event: 'mouseout' } },
		position: {
	        corner: {
	           target: 'topMiddle',
	           tooltip: 'bottomMiddle'
	...
	jQuery('.twcp').qtip({
		content: 'Get a Pro License to use these features!',
		show: { when: { target: jQuery('.twcp') } },
	    hide: { when: { target: jQuery('.twcp'), event: 'mouseout' } },
		position: {
	        corner: {
	           target: 'bottomMiddle',
	           tooltip: 'topRight'
	...
	jQuery('[name^=twcp]').each(function(){jQuery(this).attr('disabled',true);});
});
